...
export async function saveSemesterGrades(courseGrades: { id: string; grade: string }[]) {
  try {
    ...
    // Batch update course grades in DB for the Grade Sheet
    ...
    return { success: true, message: "Grade sheet updated and saved!" };
  } catch (error) {
...
export async function saveAcademicGoal(data: {
  ...
  try {
    ...
    revalidatePath('/cgpa-forecast');
    return { success: true, message: "Goal saved to planner!" };
  } catch (error) {
